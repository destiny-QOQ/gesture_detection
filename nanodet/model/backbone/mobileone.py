...
class MobileOne(nn.Module):
    ...
    def __init__(self,
        ...
        self._initialize_weights(pretrain=False)
    ...
    def _initialize_weights(self, pretrain=True):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        if pretrain:
            pretrained_state_dict = torch.load("/media/tclwh2/facepro/zhangdi/nanodet/weights/mobileone_s0_unfused.pth.tar")
            print("=> loading pretrained model ")
            self.load_state_dict(pretrained_state_dict, strict=False)


PARAMS = {
    "s0": {"width_multipliers": (0.5, 0.5, 0.5, 1.0),
           "num_conv_branches": 4},
...
def mobileone(inference_mode: bool = False,
              model_name: str = "s0", out_stages: tuple = (2,3,4)) -> nn.Module:
    """Get MobileOne model.
    :param num_classes: Number of classes in the dataset.
    :param inference_mode: If True, instantiates model in inference mode.
    :param variant: Which type of model to generate.
    :return: MobileOne model. """
    variant_params = PARAMS[model_name]
    return MobileOne(inference_mode=inference_mode, out_stages=out_stages, **variant_params)
...
if __name__ == '__main__':
    model = mobileone(inference_mode=False, model_name="s0", out_stages=(2,3,4))
