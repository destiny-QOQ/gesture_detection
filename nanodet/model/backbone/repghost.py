...
class RepGhostNet(nn.Module):
    def __init__(
        ...
        self._initialize_weights(pretrain=True)
    ...
    def forward(self, x):
        ...
        x = self.act1(x)
        outputs = []
        # for i, one in enumerate(self.blocks):
        #     x = one(x)
        #     print(f"block{i}", x.shape)
        x1 = self.blocks[:5](x)
        outputs.append(x1)
        x2 = self.blocks[5:7](x1)
        outputs.append(x2)
        x3 = self.blocks[7:](x2)
        outputs.append(x3)
    ...
    def _initialize_weights(self, pretrain=True):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        if pretrain:
            pretrained_state_dict = torch.load("/media/tclwh2/facepro/zhangdi/nanodet_v2/weights/repghostnet_1_3x_231M_76.37.pth.tar", map_location="cpu")["state_dict"]
...
if __name__ == "__main__":
    model = repghostnet_0_5x().eval()
    model = repghost_model_convert(model)
    ...
    input_data = {"data": input.detach().cpu().numpy()}
    model_sim, flag = onnxsim.simplify("rep_ghost.onnx", input_data=input_data)
    onnx.save(model_sim, "rep_ghost.onnx")