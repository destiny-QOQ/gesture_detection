...
class maxpool_pad(nn.Module):
    def __init__(self, kernel_size, stride=1, padding=0):
        super(maxpool_pad, self).__init__()
        self.maxpool = nn.MaxPool2d(kernel_size, stride, 0)

    def forward(self, x):
        ih, iw = x.size()[-2:]
        kh, kw = self.maxpool.kernel_size, self.maxpool.kernel_size
        pad_h = _calc_same_pad(ih, kh, self.maxpool.stride, 1)
        pad_w = _calc_same_pad(iw, kw, self.maxpool.stride, 1)
        x = F.pad(x, [pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2])
...
class MBConvBlock(nn.Module):
    def __init__(
        ...
        #                         bias=False,
        #                                 )
        self._depthwise_conv = Conv2dSame(in_channels=oup,
                                          out_channels=oup,
                                          groups=oup,
                                          kernel_size=k,
                                          padding=0,
...
class EfficientNetLite(nn.Module):
    def __init__(
        ...
        self.stem = nn.Sequential(
            # nn.Conv2d(3, out_channels, kernel_size=3, stride=2, padding=0, bias=False),
            Conv2dSame(3, out_channels, kernel_size=3, stride=2, padding=1, bias=False),
    ...
    def _initialize_weights(self, pretrain=True):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        if pretrain:
            pretrained_state_dict = torch.load("/media/tclwh2/facepro/zhangdi/nanodet/weights/efficientnet_lite0_re.pth")
...
def efficientnet_lite0():
    model = EfficientNetLite(model_name="efficientnet_lite0", out_stages=[2, 4, 6])
    return model
