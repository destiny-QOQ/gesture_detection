...
class Conv2dSame(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.transposed = 00
        self.output_padding = 00
        self.groups = groups

    def forward(self, x):
        ih, iw = x.shape[-2:]
        kh, kw = self.conv.weight.size()[-2:]
        pad_h = _calc_same_pad(int(ih), kh, self.conv.stride[0], self.conv.dilation[0])
        pad_w = _calc_same_pad(int(iw), kw, self.conv.stride[1], self.conv.dilation[1])
...
class ConvModule(nn.Module):
    ...
    def __init__(
        ...
        #     bias=bias,
        # )
        self.conv = Conv2dSame(in_channels, out_channels, kernel_size, stride=stride, dilation=dilation, groups=groups, bias=bias)
...
class DepthwiseConvModule(nn.Module):
    def __init__(
        ...
        #     bias=bias,
        # )
        self.depthwise = Conv2dSame(in_channels, in_channels, kernel_size, stride=stride, dilation=dilation, groups=in_channels, bias=bias)

        # self.pointwise = nn.Conv2d(
        #     in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=bias
        # )

        self.pointwise = Conv2dSame(in_channels, out_channels, 1, stride=1, bias=bias)
    ...
    def init_weights(self):
        if self.activation == "LeakyReLU":
            ...
            nonlinearity = "relu"
        kaiming_init(self.depthwise.conv, nonlinearity=nonlinearity)
        kaiming_init(self.pointwise.conv, nonlinearity=nonlinearity)
        if self.with_norm:
